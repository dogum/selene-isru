...
def test_regression_anchors() -> None:
    ...
    assert result["logistics"]["totalInfraMassKg"] / 1000 == result["logistics"]["paybackDays"]
    assert 55 <= result["logistics"]["paybackDays"] <= 62
...
def test_fixed_seed_uncertainty_anchor() -> None:
    ...
    assert result["paybackDays"]["p10"] <= result["paybackDays"]["p50"] <= result["paybackDays"]["p90"]
    assert result["secTotal"]["p10"] <= result["secTotal"]["p50"] <= result["secTotal"]["p90"]
    assert_rel(result["paybackDays"]["p50"], 58.932891221865745, 1e-12)
    assert_rel(result["secTotal"]["p50"], 24.77768664291294, 1e-12)
